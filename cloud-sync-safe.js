/* CLOUD-SYNC-SAFE-V5 — direct Supabase REST, persisted-session recovery, three-way merge, reliable Sync Now */
(function(){
  if(globalThis.__cloudSyncSafeV5Installed)return;
  globalThis.__cloudSyncSafeV5Installed=true;
  const BASE='my-bookshelf-sync-base-v5:';
  ...
  async function getSession(){
    ...
    try{
      ...
      if(stored){try{if(typeof session!=='undefined')session=stored;}catch(_){}try{if(typeof setSession==='function')setSession(stored);}catch(_){}return stored;}
    }catch(_){}
    try{
      if(globalThis.supabase?.createClient){
        if(!getSession.client)getSession.client=globalThis.supabase.createClient(SUPA,KEY,{auth:{persistSession:true,autoRefreshToken:true,storage:localStorage,storageKey:'my-bookshelf-supabase-auth'}});
        const r=await getSession.client.auth.getSession(),s=r?.data?.session;
        if(s?.user?.id&&s?.access_token){try{if(typeof session!=='undefined')session=s;}catch(_){}try{if(typeof setSession==='function')setSession(s);}catch(_){}return s;}
      }
    }catch(e){console.warn('Could not restore Supabase session',e);}
    return null;
  }
  ...
  async function refreshSession(s){
    ...
    try{
      ...
      if(n?.access_token&&n?.user?.id){try{if(typeof session!=='undefined')session=n;}catch(_){}try{if(typeof setSession==='function')setSession(n);}catch(_){}return n;}
    }catch(_){}
    return s;
  }
  ...
  async function rest(s,path,opts){
    ...
    if(r.status===401&&s.refresh_token){
      const fresh=await refreshSession({...s,access_token:null});
      if(fresh?.access_token&&fresh.access_token!==s.access_token){s.access_token=fresh.access_token;s.refresh_token=fresh.refresh_token||s.refresh_token;r=await fetch(SUPA+path,{...opts,headers:{apikey:KEY,Authorization:'Bearer '+s.access_token,'Content-Type':'application/json',...(opts?.headers||{})}});}
    }
  ...
  async function getCloud(s){
    const u=encodeURIComponent(s.user.id);
    const r=await rest(s,'/rest/v1/books?select=data,updated_at&user_id=eq.'+u+'&id=eq.library');
    return Array.isArray(r)?(r[0]||null):null;
  }
  async function writeCloud(s,data){
    const u=encodeURIComponent(s.user.id),f='user_id=eq.'+u+'&id=eq.library',now=new Date().toISOString();
    const existing=await rest(s,'/rest/v1/books?select=id&'+f);
    if(Array.isArray(existing)&&existing.length)await rest(s,'/rest/v1/books?'+f,{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify({data,updated_at:now})});
    else await rest(s,'/rest/v1/books',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify({id:'library',user_id:s.user.id,data,updated_at:now})});
    return getCloud(s);
  }
  ...
  function mergeCollection(local,cloud,base){
    ...
    for(const k of new Set([...L.keys(),...C.keys(),...B.keys()])){
      ...
      if(l===undefined){out.push(cp(c));continue;}
      if(c===undefined){out.push(cp(l));continue;}
      if(eq(l,c)){out.push(cp(l));continue;}
      if(b===undefined){out.push(cp(c));continue;}
  ...
  async function run(manual){
    ...
    try{
      ...
      if(!s?.access_token)throw new Error('Your sign-in session could not be restored.');
      const row=await getCloud(s),cloud=cp(row?.data||{}),local=getState(),base=readBase(s.user.id);
      const merged=base?merge(local,cloud,base):mergeCollection(local.books,cloud.books,[]);
      const next=base?merged:{...cloud,books:merged,fanfiction:mergeCollection(local.fanfiction,cloud.fanfiction,[]),booksToBuy:mergeCollection(local.booksToBuy,cloud.booksToBuy,[])};
      if(!eq(next,cloud)||!row?.data){
        const saved=await writeCloud(s,next);
        if(!saved?.data)throw new Error('Cloud save could not be verified.');
      ...
      console.error('V5 sync failed',e);setStatus('Sync failed — your local data is safe');
      if(manual)alert('Cloud sync could not be completed. Your local bookshelf was not replaced.\n\n'+(e.message||e));
    }finally{busy=false;}
  }
  ...
  function bindSyncButton(){
    document.addEventListener('click',function(e){
      ...
      if(t==='sync now'||t.includes('sync now')){
        e.preventDefault();e.stopImmediatePropagation();run(true);
      }
    },true);
  }
