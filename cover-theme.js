...
(() => {
  ...
  function installStyles(){if(document.getElementById(STYLE_ID))return;const style=document.createElement('style');style.id=STYLE_ID;style.textContent=`
    ...
    .book[data-cover-theme-ready="1"] .chips .chip{background:hsl(var(--cover-h),calc(var(--cover-s)*.92),calc(var(--cover-l) - 2%));color:hsl(var(--cover-h),52%,27%);border-color:hsla(var(--cover-h),58%,42%,.25)}
