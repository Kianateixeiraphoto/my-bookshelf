...
(() => {
  ...
  function rgbToHsl(r,g,b){r/=255;g/=255;b/=255;const max=Math.max(r,g,b),min=Math.min(r,g,b),d=max-min,l=(max+min)/2;let h=0,s=0;if(d){s=l>.5?d/(2-max-min):d/(max+min);switch(max){case r:h=(g-b)/d+(g<b?6:0);break;case g:h=(b-r)/d+2;break;default:h=(r-g)/d+4;}h/=6;}return{h:h*360,s:s*100,l};}
  ...
  function loadBooksToBuy(){if(document.getElementById('books-to-buy-script'))return;const script=document.createElement('script');script.id='books-to-buy-script';script.src='books-to-buy.js?v=20260819-fix5';script.defer=true;document.head.appendChild(script);}
  function loadSpiceFix(){if(document.getElementById('spice-rating-fix'))return;const script=document.createElement('script');script.id='spice-rating-fix';script.src='spice-rating-fix.js?v=20260819-fix5';script.defer=true;document.head.appendChild(script);}
  ...
  function loadCloudSyncSafe(){if(document.getElementById('cloud-sync-safe-script'))return;const script=document.createElement('script');script.id='cloud-sync-safe-script';script.src='cloud-sync-safe.js?v=20260819-fix1';script.defer=true;document.head.appendChild(script);}
